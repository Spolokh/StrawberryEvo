...
var RLANG = { //RLANG.image
	...
	image_web_link: 'Cсылка на картинку',
	...
	var Redactor = function(element, options){
		...
		this.opts = $.extend({
			...
			buttons: ['html', '|', 'formatting', '|', 'bold', 'italic', 'deleted', '|', 'unorderedlist', 'orderedlist', 'outdent', 'indent', '|',
					...
					'fontcolor', 'backcolor', '|', 'alignleft', 'aligncenter', 'alignright', 'justify', 'undo', '|', 'horizontalrule'], // 'underline', 'alignleft', 'aligncenter', 'alignright', 'justify'
			...
			activeButtonsStates: {
				b: 'bold', strong: 'bold', i: 'italic', em: 'italic', del: 'deleted', strike: 'deleted', ul: 'unorderedlist', ol: 'orderedlist', u: 'underline'
			},
			...
			modal_image_edit: String() +
				...
				'<input type="text" id="redactor_file_alt" class="redactor_input" placeholder="'+RLANG.title+'" />' +
				//'<label>' + RLANG.link + '</label>' +
				'<input type="text" id="redactor_file_link" class="redactor_input" placeholder="'+RLANG.link+'" />' +
				'<input type="text" id="redactor_file_class" class="redactor_input" placeholder="'+RLANG.class+'" />' +
				...
				'<select id="redactor_form_image_align">' +
					...
					'<option value="center">'+RLANG.center+ '</option>'+
				'</select>' +
				
				'</div>' +
				'<div id="redactor_modal_footer">' +
					'<input type="button" name="delete" id="redactor_image_delete_btn" class="redactor_modal_btn redactor_btn_modal_close" value="' + RLANG._delete + '" />'+
					'<input type="button" name="cancel" class="redactor_modal_btn redactor_btn_modal_close" value="' + RLANG.cancel + '" />'+
			...
			modal_image: String() +
				...
				'<form id="redactorInsertImageForm" method="post" action="" enctype="multipart/form-data">' +
					'<div id="redactor_tab1" class="redactor_tab">' +
						'<input type="file" name="file" id="redactor_file" />' +
					'</div>' +
	...
	Redactor.prototype = {
		...
		init: function()
		{
			...
			function afterBuild()
			{
				...
				if (typeof this.opts.plugins === 'object') // PLUGINS
				{
		...
		build: function(mobile, whendone){
			...
			if (mobile !== true){
				...
				this.$el.hide();// hide textarea

				var html = ''; // append box and frame
				if (this.textareamode){// get html
					html = this.$el.val();
				...
				html = this.paragraphy(html); // conver newlines to p
		...
		imageEdit: function(e){  
			...
			var callback = $.proxy(function() {
				...
				$('#redactor_file_class').val($el.attr('class'));
		...
		imageDelete: function(el) {
			$(el).remove(); this.modalClose(); this.syncCode();
		},
		...
		imageSave: function(el) {
			...
			var parent = $(el).parent();
			var title  = $.trim($('#redactor_file_alt').val());
			var style  = $.trim($('#redactor_file_class').val());

			$(el).attr('alt', title).attr('class', style);
			...
			{
				$(el).css({'display':'block', 'float':'none', margin:'10px auto'});
			}	
			else
			{
			...
			if (link !== '')
			{
				if ($(parent).get(0).tagName !== 'A')
				{
					$(el).replaceWith('<a title="' +title+ '" href="' +link+ '">' +this.outerHTML(el)+ '</a>');
				}
				else
				{
					$(parent).attr('href', link).attr('title', title);
				}
			}
			else
			{
				...
				if (this.opts.imageGetJson !== false)
				{
					$.getJSON(this.opts.imageGetJson, $.proxy(function(data) {
						...
						$.each(data, $.proxy(function(key, val)
						{
							...
							var img = $('<img src="' + val.thumb + '" class="redactorfolder redactorfolder' + folderkey + '" rel="' + val.file + '" title="' + thumbtitle + '" />');
		...
		_imageSet: function(json, link) ///////////////////////////////////////
		{
			...
			if (json !== false)
			{
				var html = '';
				if (link !== true)
				{	//html = '<p><img src="' + json.filelink + '" /></p>'; html = '<figure><img src="' + json.filelink + '" /></figure>';
...
(function($){
	...
	$.fn.dragupload = function(options){ // Initialization
		return this.each(function() {
				...
				if (n.nodeType === 3)
				{
					...
					if (html)
					{
						html = html.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')
									.replace(url1, '$1<a href="' + protocol + '$2">$2</a>$3').replace(url2, '$1<a href="$2">$2</a>$5');
