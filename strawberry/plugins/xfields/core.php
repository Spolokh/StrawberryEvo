...
if ($xf_inited !== true) { // Prevent "Cannot redeclare" error
  ...
  if (!function_exists('xfieldssave')){
	  ...
	  global $cutepath;
	    ...
	    $filehandle = fopen(data_directory.'/xfields.txt', "w+");
	    if (!$filehandle)
	  ...
	  $path = data_directory.'/xfields.txt';
	  ...
	  function xfieldsdatasave($data) {
	    ...
	    $filehandle = fopen(data_directory.'/xfields-data.txt', "w");
	    if (!$filehandle)
	  ...
	  function xfieldsdataload() {
	    $filecontents = file(data_directory.'/xfields-data.txt');
	    if (!is_array($filecontents))
...
?>
          ...
          <select style="width: 400px; border: 0;" name="editedxfield[2]" id="category" onchange="onCategoryChange(this.value)">
            <option value=""<?=($editedxfield[2] == "") ? " selected" : ""?>>Все</option>
            <?=$cat_options?>
            <option value="custom"<?=($editedxfield[2] != "" and !$cat_selected) ? " selected" : ""?>>Выбрать...</options>
          </select>
        <br />
...
?>
        ...
        <select style="width: 400px;" name="editedxfield[3]" id="type" onchange="onTypeChange(this.value)" />
          <option value="text"<?=($editedxfield[3] != "textarea") ? " selected" : ""?>>Одна строка</option>
          <option value="textarea"<?=($editedxfield[3] == "textarea") ? " selected" : ""?>>Несколько строк</option>
          <option value="select"<?=($editedxfield[3] == "select") ? " selected" : ""?>>Список</option>
        </select>
      </p>
      <p id="default_text">
...
?>
        <tr>
          ...
          <td <?=$rowstyle?>>
            <?=(trim($value[2]) ? htmlspecialchars($value[2]) : "Все")?>
          </td>
          <td <?=$rowstyle?>>
            <?=(($value[3] == "text") ? "Одна строка" : "")?>
            <?=(($value[3] == "textarea") ? "Несколько строк" : "")?>
            <?=(($value[3] == "select") ? "Список" : "")?>
          </td>
          <td <?=$rowstyle?>>
            <?=($value[5] != 0 ? "Да" : "Нет")?>
          </td>
