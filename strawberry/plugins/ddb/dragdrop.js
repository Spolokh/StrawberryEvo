...
var DragDrop = {
	...
	makeListContainer : function(list) {
    	...
    	var items = list.getElementsByTagName( "dfn" );
	...
	makeItemDragable : function(item) {
		...
		item.isOutside = false;   // tracks if the item is currently outside all containers
	...
	onDragStart : function(nwPosition, sePosition, nwOffset, seOffset) {
		// update all container bounds, since they may have changed on a previous drag could be more smart about when to do this
		...
		this.parentNode.onDragOver(); // item starts out over current parent
	},

	onDrag : function(nwPosition, sePosition, nwOffset, seOffset) {
		if (this.isOutside) { // check if we were nowhere
			var container = DragDrop.firstContainer; // check each container to see if in its bounds
			while (container != null) {
