...
class Cache {
	 ...
	 * @access public
	 */
	function __construct($cache = false, $dir = cache_directory, $lifetime = 3600)
	{
		$this->dir      = $dir ? $dir.'/' : '';
		$this->cache    = $cache;
	...
	/**
	 * Удаляет кэш
	 * @param string $id Если указано, то будет удаён кэш только с таким $id
	 * @return void
	 * @access public
	 */
	function delete($id = '') {
	...
	function touch_this(){
		static $touch_this;
	    $touch_this++;
		return ($touch_this == 1 ? '' : $touch_this);
	}
	...
	function _get_folder($filename){
	    ...
	    if (!is_dir($this->dir.$result['a'])){
	        @mkdir($this->dir.$result['a'], chmod);
	    }

	    if (!is_dir($this->dir.$result['a'].'/'.$result['b'])){
	        @mkdir($this->dir. $result['a'].'/'.$result['b'], chmod);
	    }

	    if (!is_dir($this->dir.$result['a'].'/'.$result['b'].'/'.$result['c'])){
	        @mkdir($this->dir. $result['a'].'/'.$result['b'].'/'.$result['c'], chmod);
	    }
	...
	function _remove_directory($dir)
	{
		...
		while (($file = readdir($handle)) !== false){
	        if ($file != '.' and $file != '..' and $file != '.htaccess'){
	            if (is_dir($dir.$file)){
	                $this->_remove_directory($dir . $file . DS);
	            } else {
	            	@unlink($dir.$file);
	            }
	        }
