...
class txtSQLCore extends txtSQL
{
	...
	function select ($arg)
	{
        ...
        if ($arg['join']){
	        $filename = "$this->_LIBPATH/$this->_SELECTEDDB/{$arg['join']['table']}";
	        if (!$rows_join = $this->_readFile($filename.'.MYD') or !$cols_join = $this->_readFile($filename.'.FRM')){
	            $this->_error(E_USER_NOTICE, 'Table "'.$arg['join']['table'].'" doesn\'t exist');
	            return false;
	        }
            ...
            foreach ($colTemp as $k => $v){
            	if (array_key_exists($k, array_reverse($cols_join)) and $k != 'primary'){
            		$cols[$k.'_before_join_'.$arg['join']['table']] = $cols_join[$k];
            	} else {
            		$cols[$k] = $cols_join[$k];
            	}
            }

            foreach ($cols_join as $k => $v){
            	$cols[$k] = $cols_join[$k];
            }

            foreach ($rows as $k => $v){
            	$rows[$k] = array_merge($rows[$k], $rows_join[$k]);
            }
		}
		 ...
		 * then select the row with the proper type (string, boolean, or integer) */
		 $selected = array_slice($selected, $arg['limit'][0], $arg['limit'][1]);
	...
	function insert ($arg)
	{
		...
		$fp = @fopen($filename.$rand.".MYD", 'w')  or $this->_error(E_USER_ERROR, 'Error opening table '.$arg['table']);
		      @flock($fp, LOCK_EX);
		      @fwrite($fp, serialize($rows)) or $this->_error(E_USER_ERROR, 'Error writing to table '.$arg['table']);
		      @flock($fp, LOCK_UN);
		      @fclose($fp);

        if (@rename($filename.$rand.".MYD", $filename.".MYD") == false){
            @unlink($filename.".MYD");
            @rename($filename.$rand.".MYD", $filename.".MYD");
        }

		$fp = @fopen($filename.$rand.".FRM", 'w')  or $this->_error(E_USER_ERROR, 'Error opening table '.$arg['table']);
		      @flock($fp, LOCK_EX);
		      @fwrite($fp, serialize($cols)) or $this->_error(E_USER_ERROR, 'Error writing to table '.$arg['table']);
		      @flock($fp, LOCK_UN);
		      @fclose($fp);

        if (@rename($filename.$rand.".FRM", $filename.".FRM") == false){
            @unlink($filename.".FRM");
            @rename($filename.$rand.".FRM", $filename.".FRM");
        }
