...
{
	define('TXTSQL_CORE_PATH', dirname(__file__));
}
...
class txtSQL
{
	...
	{
		return $this->_tableexist($table, ($db ? $db : $this->_SELECTEDDB));
	}
	...
	function _buildIf ($where, $cols)
	{
		...
		foreach ( $where as $key => $value )
		{
			...
			switch ( $type )
			{
				...
				case 1:
				case 2:
					$quotes = ( !is_numeric($val) || $cols[rtrim($col)]['type'] != 'int' ) ? '"' : '';

                	if (preg_match('/\[(.*)\]/i', $val, $match)){
                		$_tmp = array();
                		$val_arr = explode('|', $match[1]);

                		foreach ($val_arr as $val_once){
                			$_tmp[] = $f1.'$value['.$position.']'.$f2.' '.( $op == '=' ? '==' : $op ).' '.$quotes.$val_once.$quotes;
                		}

                		$query .= ' ( '.join(' || ', $_tmp).' ) ';
                	} else {
                    	$query .= ' ( '.($l_start ? '(' : '').$f1.'$value['.$position.']'.$f2.' '.( $op == '=' ? '==' : $op ).' '.$quotes.$val.$quotes.' ) '.($l_end ? ')' : '');
                    }
					break;
				...
				case 5:
                	if (preg_match('/\[(.*)\]/i', $val, $regexp)){
                        $query .= $f1.($op == '!?' ? '!' : '').'array_intersect(explode(\',\', $value['.preg_quote($position, '/').']), explode(\'|\', \''.$regexp[1].'\'))'.$f2;
                    }
			}
		...
		//echo highlight_string(var_export($query, 1), 1);
