...
/**
* acResizeImage
* Класс для изменения размеров изображений. Реализованы различные методы ресайза, кроп, 
* а также создание миниатюр.
...
class acResizeImage {
	...
	*/
	function __construct($file){ //filetype($file)
		...
		$this->setType($file) or exit("Этот файл не является картинкой");
		$this->openImage($file);
		$this->setSize();
	}
	...
	*/
	function resize($width = false, $height = false){
		...
		$newImage = imagecreatetruecolor($newSize[0], $newSize[1]); //завернуть в отдельную функцию, сжимающую изображение поэтапно
