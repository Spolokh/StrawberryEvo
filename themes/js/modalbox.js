...
Copyright Andrey Okonetchnikov (andrej.okonetschnikow@gmail.com), 2006-2007 All rights reserved.
VERSION 1.5.5 Last Modified: 09/06/2007
*/
...
Modalbox.Methods = {
	...
	options: {
		...
		width			  : 350,                // Default width in px
		height            : 280,				// Default height in px
		overflow		  : 'hidden',
		...
		closeString: "Закрыть",               // Default title attribute for close window link
		params: {},
	...
	_init: function(options) {
		...
		this.MBoverlay = Builder.node("div", {id: "MB_overlay", opacity: "0" }); //Create the overlay
		this.MBwindow = Builder.node("div", {id: "MB_window", style: "display:none"}, [ //Create the window
		...
		this.hide = this.hide.bindAsEventListener(this); //Adding event observers
	...
	alert: function(message){
		...
		Modalbox.show(html, {title: 'Alert', width: 300});
	},
	...
	_putContent: function(){ // Prepare and resize modal box for content
		if(this.options.height == this._options.height)
			...
			this._setWidth();
			this.MBcontent.setStyle({overflow: this.options.overflow, height: Element.getHeight(this.MBwindow) - Element.getHeight(this.MBheader) - 13 + 'px'});
			this.MBcontent.show();
			this.focusableElements = this._findFocusableElements();
			this._setFocus(); // Setting focus on first 'focusable' element in content (input, select, textarea, link or button)
			this.event("afterLoad"); // Passing callback
