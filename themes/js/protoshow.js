...
var protoShow = Class.create({
	...
	initialize: function(element, options) {	// Default options
	...
	play: function() {	// Role: Starts the show and initialises master timer
	...
	forward: function(transTime) {	// Role: Runs slideshow "forwards"
		this.goMaster( this.currentSlideID + 1, transTime, "forward");
	},

	backward: function(transTime) {	// Role: Runs slideshow "backwards"	
		...
		this.transitionType(this.currentSlideEle, this.nextSlideEle, {
			...
			transitionFinish :	function() {	// pass a callback to ensure play can't resume until transition has completed
				...
				if (_this.autoPlay && _this.running ) {
					// if we're autoplaying and we're not explicity stopped otherwise show Master Timer is not permitted to restart itself
	...
	setupControls: function() {
		...
		if (typeof this.protoControls === "undefined" ) {
			...
			var controlsElements =	new Element('ol', { 'class': 'proto-controls'});
			...
			controlsElements.insert(startStop,'bottom').insert(backward,'bottom').insert(forward,'bottom');
			this.element.insert(controlsElements,'bottom');	// add into DOM		
			this.protoControls = $(controlsElements);	// extend the DOM fragment		
		} 
	...
	setupNavigation: function() {
		...
		if (typeof this.protoNavigation==="undefined" ) {
			var navElements	= new Element('ol', {'class': 'proto-navigation'});			
			...
			this.slides.each(function(e, index) { // for each slide in the show create a Nav <li> using the Template above
				...
				navElements.insert(li, 'bottom');
			});

			this.element.insert(navElements, 'bottom');
			this.protoNavigation = this.element.down('.proto-navigation');			
		}
	...
	updateCaptions: function(slide) {
		...
		var nextCaption =	/*slide.down('img').readAttribute('alt') ? slide.down('img').readAttribute('alt') : */
		slide.down('img').readAttribute('alt');  //slide.dataset.caption;

		if ( typeof nextCaption !== "undefined" || nextCaption.replace(/^\s*|\s*$/g, '').length ) {		// check that the attribute has some content (not just spaces)					
			if(!this.captionsElement.visible()) {
				this.captionsElement.show(); // just check that the element is visible
			}				
	...
	runProgressTimer: function() { // Role: runs & controls the animation of the "progress timer" 
		var _this = this;
		...
		if (this.progressTimer) {// if user has set to use progress timer and the browser supports <canvas>
			...
			// use Epoch time to ensure code executes in time specified borrowed from Emile JS http://script.aculo.us/downloads/emile.pdf
			var start = (new Date).getTime();

			// we want the timer to finish slightly before the slide transitions so we shorten the duration by 1/4
